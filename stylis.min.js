...
ea=/,+\s*(?![^(]*[)])/g,fa=/ +\s*(?![^(]*[)])/g,ia=/ *[\0] */g,ka=/,\r+?/g,E=/&/g,ma=/\W+/g,la=/:global\(((?:[^\(\)\[\]]*|\[.*\]|\([^\(\)]*\))*)\)/g,da=/@(k\w+s)\s*(\S*)\s*/,V=/::?(place)/g,na=/\s+(?=[{\];=:>+*])/g,oa=/([[}=:>+*])\s+/g,pa=/(\{[^{]+?);(?=\})/g,ja=/(:+) */g,y=0,A=0,N=0,J=1,Q=1,ba=0,Z=0,T=[],P=[],x=0,aa=0,L=1,M="",D="",w="";F.use=K;F.set=ca;return F}
