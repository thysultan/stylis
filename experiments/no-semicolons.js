							...
							for (var j = 0; j < length; j++) {
								...
								if (char === 38) {
									...
									// :
										...
										// h, t, :host
										if (compact && nextcode === 104 && selector.charCodeAt(4) === 116) {
			...
			// build line by line
				...
				if (code === 13 || code === 10) {
					...
					// /
						...
						// the idea is that when we discover that
						// a line has `:` in it we lookahead to see if the next
						// non-(whitespace/comma) character is `{` if it is not
						// we can safely add semicolons to lines that don't have semicolons
						// and parse them, this will not make it to 2.0.0
						lookahead: while (caret < eof) {
							switch (styles.charCodeAt(caret+1)) {
								case 13:
								case 10:
								case 9: {
									break;
								...
								default: {
									...
									if (
										styles.charCodeAt(caret+1) !== 123 &&
										(length = buff.length) !== 0 &&
										buff.charCodeAt(length-1) !== 12
									) {
										buff += ';';
										console.log("it's not selector"+buff);
										nosemi = 1;
									}
