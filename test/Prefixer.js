...
describe('Prefixer', () => {
	...
	test('cursor', () => {
		expect(prefix(`cursor:grab`, 6)).to.equal([`cursor:-webkit-grab;`, `cursor:-moz-grab;`, `cursor:grab`].join(''))
	})
	...
	test('size', () => {
		...
		expect(prefix(`width:stretch`, 5)).to.equal([`width:fill-available`].join(''))
